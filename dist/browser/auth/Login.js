...
export class Login {
        ...
        try {
            ...
            while (iteration < maxIterations) {
                ...
                if (state === previousState && state !== 'LOGGED_IN' && state !== 'UNKNOWN') {
                    sameStateCount++;
                    this.bot.logger.debug(this.bot.isMobile, 'LOGIN', `Same state count: ${sameStateCount}/4 for state "${state}"`);
                    if (sameStateCount >= 4) {
                        this.bot.logger.warn(this.bot.isMobile, 'LOGIN', `Stuck in state "${state}" for 4 loops, refreshing page`);
    ...
    async detectCurrentState(page, account) {
        ...
        if (identityBanner && primaryButton && !passwordEntry && !results.includes('2FA_TOTP')) {
            ...
            this.bot.logger.debug(this.bot.isMobile, 'DETECT-STATE', `Get code state detected: ${codeState} (has password: ${!!account?.password})`);
        ...
        if (foundStates.includes('ERROR_ALERT')) {
            this.bot.logger.debug(this.bot.isMobile, 'DETECT-STATE', `ERROR_ALERT found - hostname: ${url.hostname}, has 2FA: ${foundStates.includes('2FA_TOTP')}`);
    ...
    async handleState(state, page, account) {
        ...
        switch (state) {
            ...
            case 'SIGN_IN_ANOTHER_WAY_EMAIL': {
                ...
                this.bot.logger.info(this.bot.isMobile, 'LOGIN', `Using ${emailSelector === this.selectors.emailIcon ? 'new' : 'old'} email icon selector`);
            ...
            case 'UNKNOWN': {
                const url = new URL(page.url());
                this.bot.logger.warn(this.bot.isMobile, 'LOGIN', `Unknown state at ${url.hostname}${url.pathname}, waiting`);
                return true;
            }
            default:
    ...
    async finalizeLogin(page, email) {
    ...
    async verifyBingSession(page) {
        ...
        try {
            ...
            for (let i = 0; i < loopMax; i++) {
                ...
                this.bot.logger.debug(this.bot.isMobile, 'LOGIN-BING', `At Bing home: ${atBingHome} (${u.hostname}${u.pathname})`);
                if (atBingHome) {
            ...
            this.bot.logger.warn(this.bot.isMobile, 'LOGIN-BING', `Verification error: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
            ...
            for (let i = 0; i < loopMax; i++) {
                ...
                if (atRewardHome) {
                    ...
                    if (isModernDashboard) {
                        ...
                        this.bot.logger.warn(this.bot.isMobile, 'GET-REWARD-SESSION', 'Modern Rewards dashboard detected. This script version may not fully support it.');
                        this.bot.logger.warn(this.bot.isMobile, 'GET-REWARD-SESSION', 'RequestToken disabled for this session (expected behavior).');
                    }
                    ...
                    if (token) {
                        ...
                        this.bot.logger.info(this.bot.isMobile, 'GET-REWARD-SESSION', `Request token retrieved: ${token.substring(0, 10)}...`);
                        return;
                    }
                    this.bot.logger.debug(this.bot.isMobile, 'GET-REWARD-SESSION', 'Token not found on page');
                }
                else {
                    this.bot.logger.debug(this.bot.isMobile, 'GET-REWARD-SESSION', `Not at reward home: ${u.hostname}${u.pathname}`);
                }
                await this.bot.utils.wait(1000);
            }
            this.bot.logger.warn(this.bot.isMobile, 'GET-REWARD-SESSION', 'No RequestVerificationToken found, some activities may not work');
        }
        catch (error) {
            throw this.bot.logger.error(this.bot.isMobile, 'GET-REWARD-SESSION', `Fatal error: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
