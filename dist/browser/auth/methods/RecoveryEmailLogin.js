...
export class RecoveryLogin {
    ...
    async fillEmail(page, email) {
        try {
            ...
            this.bot.logger.warn(this.bot.isMobile, 'LOGIN-RECOVERY', `Email input field not found with selector: ${this.textInputSelector}`);
            return false;
        }
        catch (error) {
            this.bot.logger.warn(this.bot.isMobile, 'LOGIN-RECOVERY', `Failed to fill email input: ${error instanceof Error ? error.message : String(error)}`);
            return false;
        }
    }
    async handle(page, recoveryEmail) {
        try {
            this.bot.logger.info(this.bot.isMobile, 'LOGIN-RECOVERY', 'Email recovery authentication flow initiated');
            if (recoveryEmail) {
                this.bot.logger.info(this.bot.isMobile, 'LOGIN-RECOVERY', `Using provided recovery email: ${recoveryEmail}`);
            ...
            this.bot.logger.info(this.bot.isMobile, 'LOGIN-RECOVERY', 'No recovery email provided, will prompt user for input');
            for (let attempt = 1; attempt <= this.maxManualAttempts; attempt++) {
                ...
                if (!email) {
                    this.bot.logger.warn(this.bot.isMobile, 'LOGIN-RECOVERY', `No or invalid email input received (attempt ${attempt}/${this.maxManualAttempts})`);
                ...
                if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
                    this.bot.logger.warn(this.bot.isMobile, 'LOGIN-RECOVERY', `Invalid email format received (attempt ${attempt}/${this.maxManualAttempts}) | length=${email.length}`);
                    if (attempt === this.maxManualAttempts) {
                ...
                if (!filled) {
                    this.bot.logger.error(this.bot.isMobile, 'LOGIN-RECOVERY', `Failed to fill email input field (attempt ${attempt}/${this.maxManualAttempts})`);
                    if (attempt === this.maxManualAttempts) {
