    ...
    async get(email) {
        try {
            ...
            await this.page.goto(authorizeUrl.href).catch(err => {
                this.bot.logger.debug(this.bot.isMobile, 'LOGIN-APP', `page.goto() failed: ${err instanceof Error ? err.message : String(err)}`);
            });
            ...
            while (Date.now() - start < this.maxTimeout) {
                ...
                try {
                    ...
                    this.bot.logger.debug(this.bot.isMobile, 'LOGIN-APP', `Invalid URL while polling: ${String(currentUrl)}`);
                }
                await this.bot.utils.wait(1000);
            }
            if (!code) {
                this.bot.logger.warn(this.bot.isMobile, 'LOGIN-APP', `Timed out waiting for OAuth code after ${Math.round((Date.now() - start) / 1000)}s`);
