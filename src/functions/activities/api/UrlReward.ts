    ...
    public async doUrlReward(promotion: BasePromotion, page?: Page) {
        ...
        if (!this.bot.requestToken && this.bot.rewardsVersion === 'legacy') {
            this.bot.logger.warn(
                this.bot.isMobile,
                'URL-REWARD',
                `No request token available, falling back to browser visit | offerId=${promotion.offerId}`
            )
        ...
        try {
            ...
            if (this.gainedPoints > 0) {
                ...
                this.bot.logger.warn(
                    this.bot.isMobile,
                    'URL-REWARD',
                    `Failed UrlReward with no points, falling back to browser visit | offerId=${offerId} | status=${response.status}`
                )
                await this.browserVisit(url, page)
            }
            ...
            this.bot.logger.error(
                this.bot.isMobile,
                'URL-REWARD',
                `Error in doUrlReward, falling back to browser visit | offerId=${promotion.offerId} | message=${error instanceof Error ? error.message : String(error)}`
            )
            await this.browserVisit(url, page)
        }
    }
