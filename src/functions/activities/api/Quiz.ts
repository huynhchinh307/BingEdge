        ...
        try {
            ...
            if (promotion.activityProgressMax === 80) {
                this.bot.logger.warn(
                    this.bot.isMobile,
                    'QUIZ',
                    `Detected 8-question quiz (activityProgressMax=80), falling back to browser visit | offerId=${offerId}`
                )
            ...
            if ([20, 30, 40, 50].includes(promotion.pointProgressMax)) {
                ...
                for (let i = 0; i < maxAttempts; i++) {
                    try {
                        ...
                        this.bot.logger.debug(
                            this.bot.isMobile,
                            'QUIZ',
                            `Balance delta in loop | attempt=${i + 1}/${maxAttempts} | offerId=${offerId} | oldBalance=${oldBalance} | newBalance=${newBalance} | gainedPoints=${gainedPoints} | totalGained=${totalGained}`
                        )
                        ...
                        if (totalGained >= promotion.pointProgressMax) {
                            ...
                            this.bot.logger.info(
                                this.bot.isMobile,
                                'QUIZ',
                                `Completed Quiz via API | offerId=${offerId} | attempts=${i + 1} | gainedPoints=${totalGained} | newBalance=${newBalance}`,
                                'green'
                            )
                            return
                        }
                        ...
                        this.bot.logger.error(
                            this.bot.isMobile,
                            'QUIZ',
                            `Error in Quiz loop | offerId=${offerId} | attempt=${i + 1} | message=${error instanceof Error ? error.message : String(error)}`
                        )
                    }
                }
                ...
                this.bot.logger.warn(
                    this.bot.isMobile,
                    'QUIZ',
                    `Quiz API loop finished without reaching target points, falling back to browser visit | offerId=${offerId} | totalGained=${totalGained}/${promotion.pointProgressMax}`
                )
                await this.browserVisit(url, page)
            } else {
                this.bot.logger.warn(
                    this.bot.isMobile,
                    'QUIZ',
                    `Unsupported quiz configuration, falling back to browser visit | offerId=${offerId} | pointProgressMax=${promotion.pointProgressMax}`
                )
                await this.browserVisit(url, page)
            }
        } catch (error) {
            this.bot.logger.error(
                this.bot.isMobile,
                'QUIZ',
                `Error in Quiz solver, falling back to browser visit | offerId=${offerId} | message=${error instanceof Error ? error.message : String(error)}`
            )
            await this.browserVisit(url, page)
        }
    }

    private async browserVisit(url: string, page?: Page) {
        if (!page) {
